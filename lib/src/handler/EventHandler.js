...
class EventHandler {
    ...
    hello() {
        this.client.emit(Events_1.Events.HELLO, this.payload.heartbeat_interval);
    }
    async interaction() {
        const interaction = await this.resolve.resolveInteraction(this.payload);
        this.client.emit(Events_1.Events.INTERACTION_CREATE, interaction);
    }
    async reaction() {
    ...
    async messageUpdate() {
        const message = await this.resolve.resolveMessage(this.payload);
        this.client.emit(Events_1.Events.UPDATE_MESSAGE, message);
    }
}
