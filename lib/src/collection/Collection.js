...
class Collection extends Map {
  ...
  add(obj, extra, replace) {
    const existing = this.get(obj.id); // Ifs

    if (this.limit === 0) {
      return obj instanceof this.baseObject || (obj === null || obj === void 0 ? void 0 : obj.constructor.name) === this.baseObject.name ? obj : new this.baseObject(extra, replace);
    }
  ...
  update(obj, extra, replace) {
    ...
    if (!item) return this.add(obj, extra, replace); // @ts-ignore
  ...
  toString() {
    var _this$baseObject;

    return `[Collection<${(_this$baseObject = this.baseObject) === null || _this$baseObject === void 0 ? void 0 : _this$baseObject.name}>]`;
  }
