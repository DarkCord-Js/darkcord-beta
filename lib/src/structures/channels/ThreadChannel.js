...
class ThreadChannel extends GuildChannel_1.default {
    _threadMetadata;
    members;
    constructor(_id, _client, _type, _name, _lastMessageId, _lastPinTimestamp, _position, _parentId, _topic, _guild, _permissionOverwrites, _nsfw, _rateLimitPerUser, _threadMetadata) {
    ...
    async join() {
        await this.client.requestHandler('PUT', `${Constants_1.EndPoints.channels}/${this.id}/${Constants_1.EndPoints.thread_members}/@me`);
        return this;
    }
    async leave() {
        await this.client.requestHandler('DELETE', `${Constants_1.EndPoints.channels}/${this.id}/${Constants_1.EndPoints.thread_members}/@me`);
        return this;
    }
    async edit(options) {
        let newThreadChannel = await this.client.requestHandler('PATCH', `${Constants_1.EndPoints.channels}/${this.id}`, {
        ...
        new CacheManager_1.default(this.client).manage('channels', this.id, newThreadChannel);
    }
}
